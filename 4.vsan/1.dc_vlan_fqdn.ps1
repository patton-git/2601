...
# 1. 서버 목록 최적화 (중복 호스트명 수정 및 데이터 정제)
$ServerList = @(
    ...
    @{HostName="sc-witness-02"; IP="10.10.30.12"} 
)

# [함수] CIDR에서 역방향 영역명 생성 (정교한 파싱) 
function Get-ReverseZoneName {
    param([string]$NetworkID)
    $IPPart = $NetworkID.Split('/')[0]
    $Parts = $IPPart.Split('.')
    # C클래스(24비트) 기준: 10.10.30.0 -> 30.10.10.in-addr.arpa
...
function Get-NetworkFromIP {
    ...
    $IPParts = $IP.Split('.')
    if ($IPParts.Count -ne 4) { return $null } # 잘못된 IP 형식 방어 
...
# 2. 역방향 조회 영역 자동 인식 및 생성 
Write-Host "`n--- 2단계: 역방향 조회 영역 자동 인식 및 생성 ---" -ForegroundColor Cyan
$NetworkIDs = $ServerList | ForEach-Object { Get-NetworkFromIP -IP $_.IP -Prefix 24 } | Select-Object -Unique

foreach ($NetworkID in $NetworkIDs) {
    if ($null -eq $NetworkID) { continue }
    ...
    if (!(Get-DnsServerZone -Name $ReverseZone -ErrorAction SilentlyContinue)) {
        try {
            # Add-DnsServerPrimaryZone의 -NetworkId는 10.10.10.0/24 형식을 바로 받습니다. 
            ...
            Write-Host "  ✗ 역방향 영역 생성 실패: $($_.Exception.Message)" -ForegroundColor Red
        }
    } else {
...
Write-Host "`n--- 3단계: FQDN 레코드 등록 ---" -ForegroundColor Cyan
$SuccessCount = 0
$FailureCount = 0
...
foreach ($Server in $ServerList) {
    ...
    try {
        # -CreatePtr 옵션을 사용하려면 해당 역방향 영역이 반드시 먼저 생성되어 있어야 함 
        ...
        Write-Host "  ✗ 등록 실패: $FQDN - $($_.Exception.Message)" -ForegroundColor Red
        $FailureCount++
    }
}
