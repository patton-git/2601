...
$ServerList = @(
    @{HostName="vcsa"; IP="10.10.10.10"},
    @{HostName="esx-01"; IP="10.10.10.11"},
    @{HostName="esx-02"; IP="10.10.10.12"},
#   @{HostName="storage-01"; IP="10.20.20.20"},
#    @{HostName="storage-02"; IP="10.20.20.21"},
)
...
# 1. DNS 서버 관리 모듈(RSAT) 확인 및 설치
Write-Host "--- 1단계: DNS 관리 모듈 설치 여부 확인 ---" -ForegroundColor Cyan
$DnsFeature = Get-WindowsFeature RSAT-DNS-Server
if ($DnsFeature.InstallState -ne "Installed") {
    Write-Host "DNS 관리 도구가 없습니다. 설치를 시작합니다..." -ForegroundColor Yellow
    Install-WindowsFeature RSAT-DNS-Server -IncludeManagementTools | Out-Null
    Write-Host "설치 완료." -ForegroundColor Green
} else {
    Write-Host "DNS 관리 도구가 이미 설치되어 있습니다." -ForegroundColor Green
}
